...
# create the topic we're going to write to
docker run --rm -it --net=host landoop/fast-data-dev bash
kafka-topics --create --topic traces --partitions 1 --replication-factor 1 --zookeeper 127.0.0.1:2181
# Start a console consumer on that topic
kafka-console-consumer --topic traces --bootstrap-server 127.0.0.1:9092
kafka-topics --delete --zookeeper 127.0.0.1:9092 --topic traces
kafka-console-producer --broker-list 127.0.0.1:9092 --topic traces
